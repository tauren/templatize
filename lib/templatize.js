...
var modelName = 'm';
// Name prefix to use for inner objects
var innerName = 'i';
// Wraps templatized output into a function
var prefix = 'function('+modelName+'){return \'';
var suffix = '\';}';
// Current depth level
var depth = 0;
// Current model name
...
function dotsReplacer(match, dots, value, position, full) {
  ...
  curDepth = curDepth > 0 ? curDepth : 0;
  // Construct modelName for selected depth
  var curName = curDepth ? innerName + curDepth : modelName;
  return curName+'.'+value;
}
...
function contentReplacer(match, type, value, position, full) {
  ...
  switch(type) {
    case '#each':
      value = '\'+_.map('+value.replace(reDots,dotsReplacer)+
        ', function('+(name=innerName+(++depth))+') { return \'';
      break;
    case '/each':
      value = '\';}).join(\'\')+\'';
      name = depth ? innerName+(--depth) : modelName;
      break;
    case '>':
